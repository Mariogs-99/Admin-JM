...
export const FilterContainer = ({ results, setFilters, onAdd }: FilterContainerProps) => {
  ...
  const getRooms = async () => {
    try {
      ...
      if (!Array.isArray(fetchRooms)) throw new Error("Expected an array from GetRooms()");
      setRoomsList(
        fetchRooms.map((room) => ({
          name: room.nameEs,
          code: room.roomId,
      ...
      <AnimatePresence>
        {filters && (
          <motion.div
            ...
            className="grid grid-cols-5 gap-14 bg-[#f5e79a] py-2 px-5 mt-3 mb-7"
          >
            <FilterSelectUI
              data={roomsList}
              title={
            ...
            <div className="flex flex-col justify-center">
              <label className="text-sm mb-1 flex gap-2 items-center">
                <CiSearch size={18} />
                Código reserva
              </label>
              <input
                ...
                onChange={(e) => handleFilterChange("reservationCode", e.target.value)}
              />
            </div>
